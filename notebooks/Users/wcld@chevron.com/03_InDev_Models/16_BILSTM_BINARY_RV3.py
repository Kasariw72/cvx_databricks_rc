...
def upsampling(selectColDF,isTrain,isBinary):
  if isTrain:
    if isBinary:
      ...
      newUpDF0 = selectColDF.where("RUL BETWEEN 1 AND 720")
      newUpDF0 = newUpDF0.selectExpr(dupColLabel0)
      newUpDF1 = selectColDF.where("RUL BETWEEN 1 AND 720")
...
def getDataFromCSV(sqlContext, dbFSDir,eventIds, selectedCols, isTrainSet,isBinary=True):
  ...
  selectColDF = spark.sql("SELECT * FROM RC_DATA_TMP WHERE RUL BETWEEN 1 AND 6000 ORDER BY YEAR, EVENT_ID, CYCLE")
    ...
    for trainKey in trainDataSetKeys:
      if (trainKey>=startSet and nEpoch>=startEpoch):
        if isNotSatisfacEval is True:
          ...
          normalized_train_df_new,min_max_scaler = normalizeMaxMinTrain(train_df_new,min_max_scaler)
          train_df_new = normalized_train_df_new.sort_values(['EVENT_ID','CYCLE'])
...
val_df = normalized_val_df.sort_values(['EVENT_ID','CYCLE'])
val_df = val_df.drop_duplicates(['EVENT_ID','CYCLE'], keep='last')

# COMMAND ----------

# val_df = val_df[val_df['RUL']>=1400]
# display(val_df)
...
# targetColName="LABEL1"
# sequence_length_x = 5
# sequence_cols_x= ['CYCLE','RUL','LABEL1','EVENT_ID','LUBE_OIL_PRESS']
# seq_array, label_array, nb_features, nb_out = gen_data_train_val(targetColName, val_df,sequence_length_x, sequence_cols_x)


# COMMAND ----------

# print(seq_array[1:20])
...
modelPath = "model/3_R_3_12_BDLSTM_RV8.h5"

old_weights =""

if runMode == "RELOAD":  
  a = "dbfs:/mnt/Exploratory/WCLD/"+sparkAppName+"/"+modelPath
...
model = train_hvd("BDLSTM-BI",model, trainMap,val_df,"MULTIGPUS",tf,learning_rate,min_max_scaler,isBinary,old_weights,startSet,startEpoch)
...
normalized_test_df,min_max_scaler = normalizeMaxMinTrain(test_df,min_max_scaler)
test_df = normalized_test_df.sort_values(['EVENT_ID','CYCLE'])
